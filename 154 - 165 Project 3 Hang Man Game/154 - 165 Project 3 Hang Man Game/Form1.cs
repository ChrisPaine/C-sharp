...
using System.Net;//access to web client

namespace _154___165_Project_3_Hang_Man_Game
{
    public partial class Form1 : Form
    {
        ...
        void DrawHangPost()
        {
            //add groups with no text for boxes
        ...
        void MakeLabels()
        {
            ...
            int betweenPix = 330 / chars.Length - 1;
            for (int i = 0; i < chars.Length-1; i++)
            {
                labels.Add(new Label());
                labels[i].Location = new Point((i * betweenPix) + 10, 80);//between = 40+-
                labels[i].Text = "_";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("You need to input letter","Error", MessageBoxButtons.OK,MessageBoxIcon.Error);
                return;
            }
            if (word.Contains(letter))
            {
                ...
                if (amount == 9)
                {
                    MessageBox.Show("Sorry you loose! The word was: " + word);
                    resetGame();
                }
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox2.Text == word)
            {
                ...
                MessageBox.Show("Word is wrong!","Sorry");
                DrawBodyPart((BodyParts)amount);
                amount++;
                if (amount == 9)
                {
